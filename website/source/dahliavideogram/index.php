    ...
    <div class="blog_introduction">
      ...
      <br/><br/>  <br/><br/>
       ...
       <img src="/dahliavideogram/image/demonstrated_client_view/phantom_combined_low_and_medium_quality_photo.jpeg" style="width:90%;" />
          ...
          <br/><br/>
           &nbsp;&nbsp;&nbsp;&nbsp;Remember this mode of video viewing is intended to relieve frustrations of waiting and a supposed precomputed means of working with unstable networks and or low bandwidth situations. Another way to put it is dial up users would be able to enjoy motion pictures with objectively the only frowned upon operation of the system being low quality, which is expected however not being able to watch a video for an entire day is not expected by dialup users for example.
    ...
    <div class="blog_introduction">
      ...
      <br/><br/> 
       ...
       &nbsp;&nbsp;&nbsp;&nbsp;The paradigm of this protocol is also based on sending recognizable data all while reducing file and or ram-memory reads within the computer hardware by focusing on sending one frame to multiple clients and then moving along, with low quality clients prioritized due to their proverbial handicap; 
         <br/><br/> 
         &nbsp;&nbsp;&nbsp;&nbsp;Some more considerations to speed and low latency is there can be an option for the service to have a master control program receive the video and audio stream and then dedicate a thread to spliting each frame into split quality files that can be sent to individual programs(not threads) that manage sending the low, medium, and high quality files per video frame. Regardless if each program is running on a their own computer system or if there are as many cores dedicated to each program using a system command in linux to do as such either way will reduce latency where each quality file would otherwise have had a "turn" within the logic of a single program. Likewise the client can have multiple connections, one per quality, and one to for the timing data.
           <br/><br/> 
            &nbsp;&nbsp;&nbsp;&nbsp;Having the ability to hear what occured or what was said on average is more important than seeing what happened. Therefore it should be also noted that prioritizing highquality audio streaming is important but what makes dahlia video gram different from dedicated audio streaming is it allows for almost instant low quality video streaming(and steps up to higher qualities if the connection and preferences support it) preventing fraud(or jokes) occuring such as an important and or critical piece of audio has been transmitted along with low quality video allowing the mind to know who said it or what happened, this prevents any time from being alotted to people whom might be ready to convince you what the video goes along with the audio(jerry didnt say it, i have the video right here tom said it, they both sound similar).  
           <br/><br/>
             &nbsp;&nbsp;&nbsp;&nbsp;It should also note that a checksum verification system is available even though the entire process may seem volatile. it is encouraged to have a seperate service that can communicate with the servicing video stream assist and acknowledge checksums and perhaps allow (safe) clients share their checksums as a means of allowing the public(or private group) determine if corruption is occuring or (acertained by presumption/inference) "an attack on the data stream".
       <br/><br/> 
